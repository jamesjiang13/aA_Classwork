...
def all_vowel_pairs(words)
    ...
    (0...words.length).each do |i1|
        (0...words.length).each do |i2|
            if i2 > i1
                vowels_arr = []
                joined = words[i1] + words[i2]
                joined.each_char { |char| vowels_arr << char if vowels.include?(char) }
                if vowels_arr.uniq.length == 5
                    arr << words[i1] + " " + words[i2]
                end
            end
        end
    end
